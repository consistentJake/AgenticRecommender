...
class PipelineStages:
    ...
    def stage_run_repeat_evaluation(self) -> bool:
        ...
        try:
            ...
            self.logger.info("=" * 72)
            self.logger.info(f"{'':15s} {'Round1(cuisine)':>17s}  {'LightGCN(cuisine)':>17s}  {'Round2(vendor)':>17s}")
            self.logger.info("-" * 72)
            for k in [1, 3, 5]:
                r1_hit = metrics.get(f'round1_hit@{k}', 0)
                lg_hit = metrics.get(f'lightgcn_hit@{k}', 0)
                r2_hit = metrics.get(f'hit@{k}', 0)
                self.logger.info(f"  Hit@{k}:        {r1_hit:>17.4f}  {lg_hit:>17.4f}  {r2_hit:>17.4f}")
            for k in [1, 3, 5]:
                r1_ndcg = metrics.get(f'round1_ndcg@{k}', 0)
                lg_ndcg = metrics.get(f'lightgcn_ndcg@{k}', 0)
                r2_ndcg = metrics.get(f'ndcg@{k}', 0)
                self.logger.info(f"  NDCG@{k}:       {r1_ndcg:>17.4f}  {lg_ndcg:>17.4f}  {r2_ndcg:>17.4f}")
            r1_mrr = metrics.get('round1_mrr', 0)
            lg_mrr = metrics.get('lightgcn_mrr', 0)
            r2_mrr = metrics.get('mrr', 0)
            self.logger.info(f"  MRR:           {r1_mrr:>17.4f}  {lg_mrr:>17.4f}  {r2_mrr:>17.4f}")
            self.logger.info("-" * 72)
            for k in [1, 3, 5]:
                imp_r1 = metrics.get(f'improvement_r1_to_final_hit@{k}', 0)
                imp_lg = metrics.get(f'improvement_lgcn_to_final_hit@{k}', 0)
                self.logger.info(f"  Improvement Hit@{k} (R1->Final): {imp_r1:+.4f}  (LightGCN->Final): {imp_lg:+.4f}")
            self.logger.info("-" * 72)
