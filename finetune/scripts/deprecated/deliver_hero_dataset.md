...
## Generated Training Data Location

The processed JSONL training files are generated by running:
...
**Output files:**
- `agentic_recommender/datasets/train.jsonl` - Training samples
- `agentic_recommender/datasets/eval.jsonl` - Evaluation samples  
- `agentic_recommender/datasets/test.jsonl` - Test samples
- `agentic_recommender/datasets/meta.json` - Dataset metadata

**Analysis script:**
```bash
python agentic_recommender/datasets/analyze_dataset.py
```

## Data Format for Fine-tuning

Each JSONL record follows the Alpaca format expected by `finetune/scripts/finetune_lora.py`:

```json
{
  "instruction": "Predict whether the user will purchase the candidate product. Answer only with Yes or No.",
  "input": "User's last 10 food orders:\n1. Product (day=X, time=Y, cuisine=Z, price=$W)\n...\n\nCandidate product:\n- Product Name (cuisine: X, price: $Y)\n\nShould we recommend this product to the user? Answer Yes or No.",
  "output": "Yes",
