...
This repository is a runnable bootstrap for the task described in `docs/task`: fine-tune **Qwen3-8B** with **LoRA (rank=16)** using **LLaMA-Factory** on a sequential MovieLens recommendation dataset (last 15 watched movies → Yes/No recommendation). The code covers:
